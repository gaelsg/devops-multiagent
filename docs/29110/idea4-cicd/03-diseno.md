...
**Sin soporte de forks en v1.** Aceptar PRs externas en un repo con runner self-hosted requiere además activar "Require approval for all outside collaborators" (Settings → Actions) — documentado como pendiente, no implementado porque hoy no hay colaboradores externos y agregarlo sin poder probarlo con un fork real hubiera sido una mitigación no verificada.
